...
function soundOnOf() {
  if (audioCount == 0) {
    audioCount = 1;
    audioBtn.style.backgroundImage = "url(/mokepon/assets/img/mute-btn.svg)";
    audio.play();
  } else {
    audioCount = 0;
    audioBtn.style.backgroundImage = "url(/mokepon/assets/img/audio-btn.svg)";
    audio.pause();
  }
}
...
function changeLang(btn) {
  if (btn.id == "spanish-btn") {
    logoImg.setAttribute("src", "./Assets/Mokepon-Logo-ES.svg");
    subtitle.innerHTML = "Selecciona un personaje para comenzar el viaje!";
    mainBtn.innerHTML = "Continuar";
  } else {
    logoImg.setAttribute("src", "./Assets/Mokepon-Logo-EN.svg");
